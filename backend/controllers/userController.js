...
export const forgotPassword=catchAsynchErrors(async(req,res,next)=>{
    ...
    try {
        await sendEmail({
            email:user.email,
            subject:'Bharat-Ecom Password Reset',
            message
...
export const resetPassword=catchAsynchErrors(async(req,res,next)=>{
  ...
  .digest("hex");
    ...
    if(!user) return next(new ErrorHandler("Reset Password Token Invalid or Expirerd",404));
...
export const deleteUser=catchAsynchErrors(async (req,res,next)=>{
    ...
    const user=await User.findById(req.params.id);
   if(!user) return next(new ErrorHandler("User Doen Not Exist With Given Id",404))
    await user.remove();
    res.status(200).json({
        success:true,
        user
